function CRT_GIA_kin_eq(m, Nx, ϕ, u, D, K_eq, nu_A, nu_B, nu_C, a, b, c, C_bc, C_0, N_comp, dt, T, tol)
  # boundary conditions
  BC = Array{Any}(N_comp)
  for i in 1:N_comp
      BC[i] = createBC(m)
  ...
  for t in dt:dt:T
    ...
    while (error > tol && iter < maxit)
      ...
      J_eps[1] = linearSourceTerm(createCellVariable(m, a*K_eq.*(internalCells(C_temp[1]).^(a-1)).*(internalCells(C_temp[2]).^b), BC[1]))
      J_eps[2] = linearSourceTerm(createCellVariable(m, b*K_eq.*(internalCells(C_temp[1]).^a).*(internalCells(C_temp[2]).^(b-1)), BC[2]))
      J_eps[3] = linearSourceTerm(createCellVariable(m, -c.*(internalCells(C_temp[3]).^(c-1)), BC[3]))

      RHS_reacA = constantSourceTerm(createCellVariable(m, -K_eq.*(internalCells(C_temp[1]).^a).*(internalCells(C_temp[2]).^b).*(1-a-b)+(internalCells(C_temp[3]).^c)*(1-c), BC[1]))
      RHS_reacB = constantSourceTerm(createCellVariable(m, -K_eq.*(internalCells(C_temp[1]).^a).*(internalCells(C_temp[2]).^b).*(1-a-b)+(internalCells(C_temp[3]).^c)*(1-c), BC[2]))
      RHS_reacC = constantSourceTerm(createCellVariable(m, -K_eq.*(internalCells(C_temp[1]).^a).*(internalCells(C_temp[2]).^b).*(1-a-b)+(internalCells(C_temp[3]).^c)*(1-c), BC[3]))
