...
function rec_fact(param, G, K, MT, T, BC, IC, t_exp, R_exp)
  (k_o, k_w) = param
...
model = (t_exp, param) -> rec_fact(param, G, K, MT, T, BC, IC, t_exp, R_exp)

function error_calc(param, G, K, MT, T, BC, IC, t_exp, R_exp)
  return rec_fact(param, G, K, MT, T, BC, IC, t_exp, R_exp) - R_exp
end

function error_calculation(param, param_ind, G, K, MT, T, BC, IC, t_exp, R_exp; grad_calc=true)
  eps1 = 1e-8
  error_vals = error_calc(param, G, K, MT, T, BC, IC, t_exp, R_exp)
  if grad_calc
  ...
  if grad_calc
    for j in eachindex(param_ind)
      ...
      error_val2 = error_calc(param2, G, K, MT, T, BC, IC, t_exp, R_exp)
...
function objective_function(param::Vector{Float64}, grad::Vector{Float64}, param_all, param_ind, G, K, MT, T, BC, IC, t_exp, R_exp; w=1.0)
  ...
  error_val, jac = error_calculation(param2, param_ind, G, K, MT, T, BC, IC, t_exp, R_exp, grad_calc = grad_calc)
...
obj_fun = (param, grad) -> objective_function(param::Vector{Float64}, grad::Vector{Float64}, param_all, param_ind, G, K, MT, T, BC, IC, t_sec_cor, R_oil, w=w)
...
k_o_ini = 10.0^(-1.0)*0.023*D_oil/d*((ρ_oil(c_o)*v_o*d)/μ_oil(c_o))^(0.83)*(μ_oil(c_o)/(ρ_oil(c_o)*D_oil))^(1.0/3.0)
k_w_ini = 10.0^(-0.0)*0.023*D_water/d*((ρ_water*v_w*d)/μ_water(c_w))^(0.83)*(μ_water(c_w)/(ρ_water*D_water))^(1.0/3.0)


param_all = [k_o_ini, k_w_ini]
param_ind = [1, 2]
w = ones(length(R_exp))
