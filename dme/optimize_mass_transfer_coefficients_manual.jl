...
function rec_fact(param, G, K, MT, T, BC, IC, Lx, u_inj, poros_val, PV_exp, R_exp)
  (k_o, k_w) = param
...
function obj_func(param, G, K, MT, T, BC, IC, Lx, u_inj, poros_val, PV_exp, R_exp)
    R = rec_fact(param, G, K, MT, T, BC, IC, Lx, u_inj, poros_val, PV_exp, R_exp)
  return sum(abs2, R - R_exp), R
end
...
elseif experiment == 2
    injection_rate =  4.5202702702702705e-10 # 0.00015/(3600*24) # m^3/day
    u_inj   = injection_rate/A_core # Darcy velocity
end
...
dsor_c = c -> m_sor*ones(size(c)) #irrelevant
...
~, R_heu = obj_func([k_o_ini, k_w_ini], G, K, MT, T, BC, IC, Lx, u_inj, poros_val, PV_exp, R_exp)

if experiment == 1
    runs = 5
    mult = [1e-4, 1e-3, 1e-2, 1e-1, 1e1]#linspace(1e-3, 1e1, runs)
elseif experiment == 2
    runs = 3
    mult = [1e-9, 1e-8, 1e-7]#linspace(1e-3, 1e1, runs)
end

k_o = mult.*k_o_ini
k_w = mult.*k_w_ini

error_map = zeros(length(k_o),length(k_w))
R_sims = zeros(length(PV_exp), runs^2)
run = 1
k_os = zeros(runs^2.0)
k_ws = zeros(runs^2.0)
...
for i = 1:length(k_o)
    for j = 1:length(k_w)
        rec_error, R = obj_func([k_o[i], k_w[j]], G, K, MT, T, BC, IC, Lx, u_inj, poros_val, PV_exp, R_exp)
        error_map[i,j] = rec_error
        R_sims[:,run] = R
        k_os[run] = k_o[i]
        k_ws[run] = k_w[j]
        run += 1
...
#Visualizing
#m_plot = createMesh2D(length(k_o), length(k_w), maximum(k_o), maximum(k_w))
#error_plot = createCellVariable(m_plot, error_map)
#figure()
xlab =  10.^(floor(log(abs(k_o))./log(10)))
ylab =  10.^(floor(log(abs(k_w))./log(10)))
subplots()
pcolormesh(error_map, cmap = "viridis")
xticks((1:runs)-0.5, xlab)
yticks((1:runs)-0.5, ylab)
colorbar()
title("Sum of squared error, "L"$||R-R_{lab}||_2$")
xlabel(L"$k_o$ (order of magnitude)")
ylabel(L"$k_w$ (order of magnitude)")
savefig(string("Optimization Results/experiment", experiment, ".png"))

figure()
legString = ["Experimental", "Heuristic"]
plot(PV_exp, R_exp.*100)
plot(PV_exp, R_heu.*100)
for i = 1:(run-1)
    plot(PV_exp, R_sims[:,i].*100, linestyle="--", alpha=0.3)
    #legString = push!(legString, "\$k_o=\$$(@sprintf("%.1e", k_os[i])), \$k_w=\$$(@sprintf("%.1e", k_ws[i]))")
end
#plot(PV_exp_DME, R_sims[:,indmin(error_map[:])].*100)
